...
class LangChainOpenAIInterface:
    ...
    def get_llm_chat_client(
        self, model_name="gpt-4o-mini", temperature=0.0, **kwargs
    ) -> ChatOpenAI:
        try:
            ...
            logger.info(
                f"LangChainOpenAIInterface initialized with model: {model_name}"
            )
            return llm_client
    ...
    def get_embeddings_client(
        self, model_name="text-embedding-ada-002", **kwargs
    ) -> OpenAIEmbeddings:
        try:
            embeddings_client = OpenAIEmbeddings(
                model=model_name, api_key=self.openai_api_key, **kwargs
            )

            logger.info(
                f"OpenAI Embeddings client initialized with model: {model_name}"
            )
